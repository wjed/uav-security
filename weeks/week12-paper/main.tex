...
\textit{The attack defeats the undefended and the naively weighted systems.} FedAvg leaves $+0.2415$ of lift and accuracy-weighted FedAvg $+0.3036$, confirming that weighting by a self-reported metric is worse than not weighting at all. Both also lose spoofing recall badly ($0.3641$ and $0.3524$ against an honest $0.5292$), because the poisoned updates degrade the detector as well as installing the backdoor.

\textit{Multi-Krum is a genuinely competitive baseline under these conditions, and we do not claim otherwise.} It reaches $+0.0061 \pm 0.0041$ lift, which is statistically indistinguishable from our behavioral trust alone at $+0.0039 \pm 0.0047$, and it does so at $1.4$~ms of server time against our $38.0$~ms. Any claim that a behavioral probe is necessary to stop this attack in this setting would be contradicted by our own table. What the full defense adds over Multi-Krum is a nominal further reduction to $-0.0265$ and the best retained utility of any rule tested (recall $0.5560$, F1 $0.6087$, both above the honest baseline's $0.5292$ and $0.5944$), but at three seeds we do not claim that lift difference is resolved.
...
\textit{FLTrust, the closest prior root-of-trust defense, does not close the attack here.} It removes about two-thirds of it ($+0.0787 \pm 0.0176$) but leaves a clear residual, and its clean accuracy ($0.6962$) and recall ($0.4687$) fall \emph{below} the honest no-attack baseline, because rescaling every update to the server update's norm throttles honest clients along with attackers. It is also the most expensive rule tested at $204.4$~ms per round, roughly five times ours, since it trains a server model on the root set every round. The mechanism behind its residual is visible in its trust: it assigns the compromised clients $0.0390$, about $39\%$ of uniform, and flags them in only $84.7\%$ of client-rounds. Only $40\%$ of each attacker's spoofed rows are poisoned, so most of its gradient is ordinary learning that agrees with the server's direction, and the backdoor occupies a small subspace of a $3{,}329$-parameter update. A cosine similarity taken over the whole parameter vector is a global statistic that a localized blind spot barely rotates. The behavioral probe does not measure direction at all: it measures what the model \emph{does} on the slice where a backdoored model must differ, and separates the groups completely (attacker trust $0.0001$, detection $100\%$).
...
At the most severe level tested ($\alpha = 1$) the base detector has itself degraded to $0.232$ honest recall and $0.903$ honest BSR, so lift again has little headroom and we do not use that row to rank defenses. It is reported for completeness and because it marks where this federated configuration stops functioning at all, which is consistent with the ceiling study of Section~\ref{subsec:ceiling}.

\begin{table*}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Non-IID Sweep Over Client Class-Ratio Skew (mean $\pm$ std, $n=3$). Every Quantity the Review Requested Is Reported. Uniform Trust Is $0.100$. Rules Without a per-Client Weight Have No Detection or False-Flag Rate. The $\alpha=1$ Block Is Reported for Completeness but Not Used to Rank Rules, Because Its Honest Baseline Has Already Degraded to $0.232$ Recall.}
\label{tab:noniid}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
...
\toprule
\textbf{Condition} & \textbf{Method} & \textbf{Clean} & \textbf{Recall} & \textbf{BSR} & \textbf{Lift} &
\textbf{Detect} & \textbf{False-Flag} & \textbf{Atk trust} & \textbf{Hon trust} \\
\midrule
IID & Honest FedAvg (no attack) & $0.7112$ & $0.5292$ & $0.6367$ & $+0.0000$ & --- & --- & --- & --- \\
    & FedAvg (attacked)         & $0.6932$ & $0.3641$ & $0.8782$ & $+0.2415$ & --- & --- & --- & --- \\
    & Coordinate-wise median    & $0.7101$ & $0.5008$ & $0.7013$ & $+0.0646$ & --- & --- & --- & --- \\
    & FLTrust                   & $0.6962$ & $0.4687$ & $0.7154$ & $+0.0787$ & $84.7\%$ & $0.0\%$ & $0.0390$ & $0.1152$ \\
    & Behavioral trust (ours)   & $0.7112$ & $0.5307$ & $0.6406$ & $+0.0039$ & $100\%$ & $0.3\%$ & $0.0001$ & $0.1250$ \\
    & Trust + median (ours)     & $0.7143$ & $0.5560$ & $0.6101$ & $-0.0265$ & $100\%$ & $0.3\%$ & $0.0001$ & $0.1250$ \\
\midrule
Skew      & Honest FedAvg (no attack) & $0.6998$ & $0.4937$ & $0.6788$ & $+0.0000$ & --- & --- & --- & --- \\
$\alpha{=}10$ & FedAvg (attacked)     & $0.6868$ & $0.3461$ & $0.9370$ & $+0.2583$ & --- & --- & --- & --- \\
(mild)    & Coordinate-wise median    & $0.7035$ & $0.4487$ & $0.7498$ & $+0.0710$ & --- & --- & --- & --- \\
          & FLTrust                   & $0.6919$ & $0.4639$ & $0.7329$ & $+0.0542$ & $65.3\%$ & $8.7\%$ & $0.0429$ & $0.1143$ \\
          & Behavioral trust (ours)   & $0.6895$ & $0.3758$ & $0.9162$ & $+0.2374$ & $0.0\%$ & $0.3\%$ & $0.0950$ & $0.1013$ \\
          & Trust + median (ours)     & $0.7035$ & $0.4555$ & $0.7435$ & $+0.0647$ & $8.3\%$  & $2.1\%$ & $0.0942$ & $0.1014$ \\
\midrule
Skew      & Honest FedAvg (no attack) & $0.6903$ & $0.4446$ & $0.7388$ & $+0.0000$ & --- & --- & --- & --- \\
$\alpha{=}3$ & FedAvg (attacked)      & $0.6733$ & $0.2983$ & $0.9843$ & $+0.2455$ & --- & --- & --- & --- \\
(moderate) & Coordinate-wise median   & $0.6897$ & $0.3609$ & $0.8434$ & $+0.1046$ & --- & --- & --- & --- \\
          & FLTrust                   & $0.6880$ & $0.4468$ & $0.7534$ & $+0.0146$ & $59.7\%$ & $22.6\%$ & $0.0434$ & $0.1141$ \\
          & Behavioral trust (ours)   & $0.6739$ & $0.2979$ & $0.9870$ & $+0.2482$ & $0.0\%$ & $0.0\%$ & $0.1025$ & $0.0994$ \\
          & Trust + median (ours)     & $0.6897$ & $0.3618$ & $0.8390$ & $+0.1002$ & $0.0\%$ & $0.0\%$ & $0.1023$ & $0.0994$ \\
\midrule
Skew      & Honest FedAvg (no attack) & $0.6473$ & $0.2320$ & $0.9027$ & $+0.0000$ & --- & --- & --- & --- \\
$\alpha{=}1$ & FedAvg (attacked)      & $0.6354$ & $0.1195$ & $0.9986$ & $+0.0959$ & --- & --- & --- & --- \\
(severe,  & Coordinate-wise median    & $0.6455$ & $0.1428$ & $0.9671$ & $+0.0644$ & --- & --- & --- & --- \\
baseline  & FLTrust                   & $0.6635$ & $0.4325$ & $0.7836$ & $-0.1191$ & $66.7\%$ & $31.9\%$ & $0.0401$ & $0.1150$ \\
degraded) & Behavioral trust (ours)   & $0.6389$ & $0.1256$ & $0.9980$ & $+0.0953$ & $0.0\%$ & $0.0\%$ & $0.1054$ & $0.0987$ \\
          & Trust + median (ours)     & $0.6495$ & $0.1580$ & $0.9594$ & $+0.0567$ & $0.0\%$ & $0.0\%$ & $0.1060$ & $0.0985$ \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Dead-Zone Sweep Under Skew (mean $\pm$ std, $n=3$), Testing Whether the Trust Failure Is a Threshold Choice. Lowering $\tau$ Partially Recovers Detection, Confirming the Mechanism, but Never Approaches the IID Figure and Costs Honest False Flags. Under IID the Same Change Is Pure Loss.}
\label{tab:tau}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
\toprule
\textbf{Condition} & \textbf{$\tau$} & \textbf{Lift} & \textbf{Attacker detect} & \textbf{Honest false-flag} \\
\midrule
IID              & $0.5$ & $-0.0122$ & $100\%$   & $6.3\%$  \\
                 & $1.0$ & $-0.0212$ & $100\%$   & $4.9\%$  \\
                 & $2.0$ & $-0.0265$ & $100\%$   & $0.3\%$  \\
\midrule
Skew $\alpha{=}10$ & $0.5$ & $+0.0457$ & $38.9\%$ & $11.8\%$ \\
                 & $1.0$ & $+0.0470$ & $26.4\%$ & $6.9\%$  \\
                 & $2.0$ & $+0.0647$ & $8.3\%$  & $2.1\%$  \\
\midrule
Skew $\alpha{=}3$ & $0.5$ & $+0.0851$ & $15.3\%$ & $7.6\%$  \\
                 & $1.0$ & $+0.0902$ & $1.4\%$  & $1.7\%$  \\
                 & $2.0$ & $+0.1002$ & $0.0\%$  & $0.0\%$  \\
\bottomrule
\end{tabular}
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Centralized Detector Ceiling. All Rows Are Trained on the Full Client Pool With No Federation, No Attack and No Privacy Constraint, so They Upper-Bound What the Federated System Could Reach. The Final Row Is the Paper's Federated Baseline.}
\label{tab:ceiling}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
...
\toprule
\textbf{Model} & \textbf{Clean} & \textbf{Recall} & \textbf{F1} & \textbf{Bal.\ Acc} & \textbf{BSR} \\
\midrule
Logistic regression                & $0.6094$ & $0.2241$ & $0.3146$ & $0.5452$ & $0.9394$ \\
MLP $64\text{-}32\text{-}16$ (paper model) & $0.8728$ & $0.9073$ & $0.8509$ & $0.8786$ & $0.3779$ \\
MLP $256\text{-}128\text{-}64$     & $0.9192$ & $0.9506$ & $0.9040$ & $0.9244$ & $0.3488$ \\
MLP $512\text{-}256\text{-}128\text{-}64$ & $0.9245$ & $0.9742$ & $0.9117$ & $0.9328$ & $0.3331$ \\
Random forest (400 trees)          & $0.9503$ & $0.9595$ & $0.9392$ & $0.9519$ & $0.1037$ \\
Histogram gradient boosting        & $\mathbf{0.9615}$ & $\mathbf{0.9929}$ & $\mathbf{0.9538}$ & $\mathbf{0.9667}$ & $0.1213$ \\
\midrule
\textit{Federated honest FedAvg (paper baseline)} & $0.7112$ & $0.5292$ & $0.5944$ & $0.6809$ & $0.6367$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}l ccc@{}}
\toprule
\textbf{Defense} & \textbf{Server ms/round} & \textbf{\% of sequential} & \textbf{\% of parallel} \\
\midrule
Behavioral trust $+$ median (ours) & $34.0$  & $0.98\%$ & $3.40\%$  \\
FLTrust \cite{cao2021fltrust}      & $168.3$ & $4.88\%$ & $16.86\%$ \\
\midrule
\multicolumn{4}{@{}l}{\textit{Scaling of our server cost (ms/round, and \% of a parallel round)}} \\
Clients $5 \to 40$        & $19.3 \to 139.1$ & --- & $5.6\% \to 34.9\%$ \\
Probe features $2 \to 8$  & $13.2 \to 35.6$  & --- & $3.5\% \to 9.9\%$  \\
Root rows $600 \to 6{,}000$ & $25.5 \to 37.1$ & --- & $6.2\% \to 10.3\%$ \\
\bottomrule
\end{tabular}
\end{table}
