...
costs 1.1\% of round time with no additional client-side work. All results are obtained on a public
...
All experiments are implemented in Python with PyTorch and scikit-learn and run on CPU. Every table and figure in this paper is produced by a notebook or script in the project repository and exported to a CSV or PNG, and the reported values are read from those exports rather than transcribed, so no figure or table can disagree with the run that produced it. The dataset is publicly available \cite{aissou2022dataset}.
