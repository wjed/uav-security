...
\begin{itemize}
    ...
    \item We demonstrate the attack empirically: across three seeds it raises backdoor success by $0.2415$ over the honest baseline, and $0.3036$ when reported accuracy is inflated, while clean accuracy falls by less than two points.
    ...
    \item We evaluate the defense across three seeds with a trigger-generalization study over five features and a mixed trigger, a defense-aware adaptive attacker, a sensitivity sweep over all three defense hyperparameters, a quantitative false-flag analysis, and a measured cost analysis on both sequential and parallel round denominators. Under IID clients the defense reduces attack-induced backdoor lift to a level statistically indistinguishable from the honest baseline, at a 0.3\% honest false-flag rate and 34.0~ms of server-side computation per round, with no client-side cost.
    \item We report, rather than omit, the two settings where the defense does not hold: Multi-Krum matches it under IID data, and under client class-ratio skew the behavioral probe stops firing altogether, leaving the coordinate-wise median to carry the result.
\end{itemize}
...
\label{eq:robustagg}
\end{equation}
The two layers play different roles. The trust weights identify and down-weight the compromised clients from behavior; the coordinate-wise median bounds the damage of any single mis-scored update, and is the safety net if the trust score is ever wrong in a round. Behavioral trust provides most of the measured protection in the evaluated setting, while coordinate-wise median is retained as a robust aggregation backstop. Section~\ref{subsec:mediannec} tests that backstop directly by degrading the trust score until it fails.
...
An internal ablation shows which of our own layers contributes, but it cannot show that the result is worth having over an existing defense. Table~\ref{tab:benchmark} and Fig.~\ref{fig:baselines} therefore run every baseline the review asked for on the identical data split, attack, seeds and metrics: FedAvg, accuracy-weighted FedAvg, coordinate-wise median, trimmed mean, Krum, Multi-Krum, FLTrust~\cite{cao2021fltrust}, and both of our own variants. Nothing is quoted from another paper. Every row is one execution of one pipeline, so the comparison isolates the aggregation rule rather than the experimental setup.

This table also absorbs the layer ablation of the previous draft, so every
quantity now comes from one execution instead of two. Two points carry over.
Coordinate-median-only leaves a real residual ($+0.0646$): a median resists
scaled updates, but two clients of ten can still drag many coordinates. And
accuracy-weighted FedAvg is the only rule the inflation variant affects; every
rule that ignores self-reported accuracy, ours included, is numerically
unchanged by it, because inflation has no channel through which to act. That is
the central evidence that self-reported accuracy is the wrong basis for
aggregation weight: a defense that never reads it is immune by construction
rather than by tuning.

Three further results matter, and the second is not favorable to us.
...
We also state plainly what this table does \emph{not} establish. Behavioral
trust alone ($+0.0039$) and the full defense ($-0.0265$) are not separated at
three seeds; the difference sits inside the run-to-run spread. The table
supports the claim that behavioral trust supplies most of the protection in
this setting, not that the median layer is independently necessary.
Section~\ref{subsec:mediannec} tests that question directly.
...
\begin{table*}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Comparison With Published Baselines on an Identical Pipeline (mean $\pm$ std, $n=3$). ``Detect'' and ``False-Flag'' Are the Fractions of Attacker and Honest Client-Rounds Held Below Half the Uniform Weight; Krum Selects Rather Than Scores, so a False-Flag Rate Is Not Defined for It.}
\label{tab:benchmark}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{@{}l rrrr rr rr r@{}}
\toprule
...
\caption{Non-IID Sweep Over Client Class-Ratio Skew (mean $\pm$ std, $n=3$), Reporting Each Quantity the Review Requested. Uniform Trust Is $0.100$. A Fourth, More Severe Condition Is in the Exported CSV.}
\label{tab:noniid}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{@{}ll rrrr rr rr@{}}
\toprule
...
\midrule
\multirow{6}{*}{IID}
    & Honest FedAvg (no attack) & $0.7112$ & $0.5292$ & $0.6367$ & $+0.0000$ & --- & --- & --- & --- \\
...
\midrule
\multirow{6}{*}{\shortstack[l]{Skew\\$\alpha{=}10$\\(mild)}}
          & Honest FedAvg (no attack) & $0.6998$ & $0.4937$ & $0.6788$ & $+0.0000$ & --- & --- & --- & --- \\
          & FedAvg (attacked)         & $0.6868$ & $0.3461$ & $0.9370$ & $+0.2583$ & --- & --- & --- & --- \\
          & Coordinate-wise median    & $0.7035$ & $0.4487$ & $0.7498$ & $+0.0710$ & --- & --- & --- & --- \\
...
\midrule
\multirow{6}{*}{\shortstack[l]{Skew\\$\alpha{=}3$\\(moderate)}}
          & Honest FedAvg (no attack) & $0.6903$ & $0.4446$ & $0.7388$ & $+0.0000$ & --- & --- & --- & --- \\
          & FedAvg (attacked)         & $0.6733$ & $0.2983$ & $0.9843$ & $+0.2455$ & --- & --- & --- & --- \\
          & Coordinate-wise median    & $0.6897$ & $0.3609$ & $0.8434$ & $+0.1046$ & --- & --- & --- & --- \\
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Does the Median Layer Help When Trust Scoring Degrades? (mean, $n=3$). The Final Two Rows Are the Skew Conditions of Section~\ref{subsec:noniid}, Where the Trust Layer Fails Outright.}
\label{tab:mediannec}
\centering
\footnotesize
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.2}
\caption{Full Operating Point on the $30{,}000$-Row Test Set ($18{,}000$ Authentic, $12{,}000$ Spoofed; mean, $n=3$). Confusion Counts Are Rounded From the Mean Rates. FAR Is the False-Alarm Rate on Authentic Rows.}
\label{tab:operating}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\toprule
\textbf{Method} & \textbf{Prec.} & \textbf{Rec.} & \textbf{F1} & \textbf{Bal.\ Acc} & \textbf{FAR}
& \textbf{TP} & \textbf{FN} & \textbf{FP} & \textbf{TN} \\
\midrule
Honest FedAvg (no attack) & 0.6782 & 0.5292 & 0.5944 & 0.6809 & 0.1675 & $6350$ & $5650$ & $3015$ & $14985$ \\
FedAvg (attacked) & 0.7352 & 0.3641 & 0.4869 & 0.6383 & 0.0874 & $4369$ & $7631$ & $1573$ & $16427$ \\
FLTrust & 0.6730 & 0.4687 & 0.5522 & 0.6583 & 0.1522 & $5624$ & $6376$ & $2740$ & $15260$ \\
\textbf{Trust + median (ours)} & 0.6732 & 0.5560 & 0.6087 & 0.6879 & 0.1803 & $6672$ & $5328$ & $3245$ & $14755$ \\
\bottomrule
\end{tabular}
\end{table}
...
The three studies that follow (trigger generalization, adaptive attacker, and
the defense-side sweep) were executed as separate runs with their own exported
CSVs, so their figures for the shared configurations differ from
Table~\ref{tab:benchmark} in the last decimal, for example $-0.0253$ against
$-0.0265$ for the full defense. The differences are far inside the seed spread;
we report each study's own measurement rather than silently harmonising them.
...
Table~\ref{tab:operating} gives the full operating point the review asked for, including precision, false-alarm rate and confusion counts, for the honest baseline, the undefended attack, the nearest baseline and our defense.
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Centralized Detector Ceiling. Every Row Is Trained on the Full Client Pool With No Federation, No Attack and No Privacy Constraint, so All of Them Upper-Bound the Federated System.}
\label{tab:ceiling}
\centering
\footnotesize
\setlength{\tabcolsep}{3.5pt}
...
\caption{Centralized detector ceiling, spoofing recall on the held-out test set. Every centralized non-linear model far exceeds the federated baseline (dotted); only logistic regression, which cannot represent the decision boundary, falls below it. The weak recall reported throughout this paper therefore reflects the federated training configuration, not the separability of the feature set.}
\label{fig:ceiling}
\end{figure}
...
\begin{table}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Server-Side Cost on Both Denominators (ten clients, mean of three timed rounds). A Parallel Round Is Bounded by the Slowest Client and Is the Realistic Denominator.}
\label{tab:cost}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
