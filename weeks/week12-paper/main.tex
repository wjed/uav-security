...
survives a defense-aware adaptive attacker, holds its
protection across a sixteen-fold range of its own gate parameter, and
costs 1.1\% of round time with no additional client-side work. Against
FLTrust, the nearest published defense sharing our root-of-trust
premise and reimplemented on the identical pipeline, the behavioral
probe is decisively stronger: FLTrust leaves $+0.0787$ of backdoor lift
and costs clean accuracy relative to the honest baseline, because a
cosine similarity taken over the whole update is only mildly perturbed
by a backdoor confined to a small subspace. All results are obtained on a public
...
\subsection{Comparison with the Nearest Benchmark}
\label{subsec:benchmark}
An internal ablation shows that each of our own layers contributes, but it cannot show that the resulting defense is worth having over an existing one. Table~\ref{tab:benchmark} and Fig.~\ref{fig:benchmark} therefore compare against FLTrust \cite{cao2021fltrust} on the identical pipeline, attack, root set, and seeds. All four rows are produced by a single execution, so they are internally consistent; the honest, attack, and full-defense rows reproduce their Table~\ref{tab:ablation} counterparts to within run-to-run variation (honest BSR $0.6367$ versus $0.6368$; our lift $-0.0265$ versus $-0.0253$), which is also a check that the benchmark harness matches the one used elsewhere in the paper.

FLTrust is a real defense here, not a straw man: it cuts backdoor lift from $+0.2415$ to $+0.0787$, removing about two-thirds of the attack, and it never pushes an honest client below half its uniform weight. But it does not close the attack, and it pays for what it does remove. Its clean accuracy ($0.6962$) and spoofing recall ($0.4687$) both land \emph{below} the honest no-attack baseline ($0.7112$ and $0.5292$), whereas ours land at or above it ($0.7143$, $0.5560$). The reason is that FLTrust rescales every client update to the norm of the server's own update, which is exactly what defuses the model-replacement scaling in Eq.~\ref{eq:boost}, but it applies that throttle to honest clients too and slows their contribution.

The mechanism behind the residual is visible in the trust column. FLTrust assigns the compromised clients $0.0390$, about $39\%$ of uniform: it correctly regards them as suspicious, but not decisively so. This is the expected behavior of a direction-based score against this attack. Only $40\%$ of each attacker's spoofed rows are poisoned, so most of its gradient is ordinary learning that agrees with the server's direction, and the backdoor occupies a small subspace of a $3{,}329$-parameter update. A cosine similarity taken over the whole parameter vector is a global statistic, and a localized blind spot barely rotates it. Our probe does not measure direction at all; it measures what the model \emph{does} on the one slice where a backdoored model must behave differently, and that separates the two groups completely (attacker trust $0.0001$). Fig.~\ref{fig:benchmark} shows the same contrast over training: FLTrust starts near uniform and decays gradually across twelve rounds, while the behavioral probe is decisive in the first round.

The honest-cohort column is the one place FLTrust is ahead, and the comparison is reported on a single definition applied to both defenses, namely the fraction of honest client-rounds given less than half the uniform weight. On that definition FLTrust scores $0.0\%$ and ours $0.3\%$, i.e.\ one honest client-round out of $288$. This is a genuine, if small, cost of a sharper gate. It matches the $0.3\%$ separately characterized in Section~\ref{subsec:fp} under that section's own flagging definition, where no honest client is ever driven to zero trust. The trade is that one mildly down-weighted honest client-round, against $0.105$ of backdoor lift removed.

\begin{table}[!t]
\renewcommand{\arraystretch}{1.25}
\caption{Comparison With FLTrust on an Identical Pipeline (mean $\pm$ std, $n=3$). Trust Is Normalized so Uniform Is $0.100$; ``Honest DW'' Is the Fraction of Honest Client-Rounds Given Less Than Half the Uniform Weight.}
\label{tab:benchmark}
\centering
\footnotesize
\setlength{\tabcolsep}{3pt}
...
\toprule
\textbf{Method} & \textbf{Clean} & \textbf{Recall} & \textbf{Lift} & \textbf{Atk trust} & \textbf{Honest DW} \\
\midrule
Honest FedAvg          & $0.7112$ & $0.5292$ & $+0.0000$ & ---      & ---      \\
Attack (FedAvg)        & $0.6932$ & $0.3641$ & $+0.2415$ & ---      & ---      \\
FLTrust \cite{cao2021fltrust} & $0.6962$ & $0.4687$ & $+0.0787$ & $0.0390$ & $0.0\%$ \\
\textbf{Ours (D2)}     & $\mathbf{0.7143}$ & $\mathbf{0.5560}$ & $\mathbf{-0.0265}$ & $\mathbf{0.0001}$ & $0.3\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Backdoor lift before (orange) and after (green) the defense, per trigger setting. Each connector spans one trigger's undefended and defended lift, so its length is the backdoor the defense removes; markers are means over three seeds and whiskers are $\pm 1$ std. One fixed D2 configuration; the black line at zero marks ``the attacker gains nothing.''}
\label{fig:trigger}
\end{figure}
...
\caption{Adaptive attacker. Undefended backdoor lift (orange, dashed) and defended backdoor lift (green) on the left axis, with mean attacker trust (purple, dotted) on the right, versus evasion strength $\lambda$; mean of three seeds with $\pm 1$ std. As $\lambda$ grows the attacker's trust climbs toward uniform, so it does evade the probe more successfully, but its undefended backdoor collapses from $+0.2457$ to $-0.48$ over the same range: the two objectives are opposed. Defended lift stays negative throughout.}
\label{fig:adaptive}
\end{figure}
...
performance depends on tuning them. Table~\ref{tab:sensitivity} and
Fig.~\ref{fig:sensitivity} sweep each one across three seeds while holding the
other two at D2.
...
\begin{figure}[!t]
\centering
\includegraphics[width=3.45in]{figures/fig_defense_sensitivity.png}
\caption{Defense-side sensitivity. Backdoor lift (green, left axis, $\pm 1$ std band over three seeds) and honest false-positive rate (purple, right axis) against each defense hyperparameter, with the other two held at D2. The black line at zero marks ``the attacker gains nothing''; the undefended attack sits at $+0.246$, far above the plotted range, and is annotated rather than drawn.}
...
\section{Discussion}
The results support a single narrative. The vulnerability is real and specific: accuracy-weighted aggregation, adopted in recent UAV-FL work to reward good clients, is a second attack lever, and inflation raises backdoor lift from $+0.2457$ to $+0.3036$. The defense closes it by refusing to trust self-report and judging clients on behavior the coordinator can verify, which makes inflation inert by construction rather than by tuning. The layer ablation shows the combination is necessary: median-only leaves a quarter of the attack and trust-only lacks a safety net against a mis-scored round, while together they drive the advantage to zero at no utility cost. The benchmark comparison shows the result is not merely internal bookkeeping: FLTrust, given the same root set and the same information, still leaves $+0.0787$ of lift while costing clean accuracy, because scoring a client by the direction of its whole update is the wrong instrument for a backdoor that occupies a small subspace. What distinguishes the behavioral probe is not that it has more information but that it asks a more specific question of the same information. The trigger-generalization and adaptive-attacker studies address the two hardest questions a reviewer can ask: the defense holds when the trigger moves to a feature it was never told about, including a mixed trigger, and a defense-aware attacker cannot evade the probe without destroying its own backdoor. The sensitivity sweep adds a deployability argument to the security one: once the dead-zone removes the spurious penalties on honest clients, backdoor lift is flat and negative across a sixteen-fold range of the gate parameter, so an operator does not have to find a narrow setting to be protected. The parameter that does require a considered choice is the dead-zone width itself, and the sweep locates its knee rather than leaving it to judgment.

Several limitations bound these claims and are stated plainly, together with why each does or does not reach the central result.

The dataset is the clearest of them. It is a single software-defined-radio receiver on a mobile platform, not a fleet, and it is a simplified public collection rather than a large operational corpus; we use it because no public multi-UAV GPS spoofing dataset with labeled attacks exists. Two things bound how far that reaches. The claim under test is a property of the \emph{aggregation rule}, not of the signal domain: the attack works because Eq.~\ref{eq:weight} multiplies an attacker-chosen scalar into its own influence, and the defense works because a backdoored model must answer a probe differently from an honest one. Neither mechanism refers to GPS. What the dataset does bound is the absolute detection numbers, which is precisely why every headline result is reported as backdoor \emph{lift} against each seed's own honest baseline rather than as an absolute rate: a stronger detector moves the baseline and the attacked run together, and lift is invariant to that shift. Relatedly, the base detector is only moderately strong here (honest spoofing recall about $0.53$), so a better detector is an orthogonal improvement rather than a confound. What would genuinely test the result is a second, independent signal domain, and that is the single most valuable extension of this work.

Four narrower limitations remain. First, the evaluation uses an IID partition across ten clients with two attackers; non-IID data would widen the honest trust spread and is the setting most likely to stress the $0.3\%$ false-positive rate, making it the natural next study. Second, the trigger-agnostic claim is scoped to the probed feature set, though Table~\ref{tab:features} shows that scope is generous: the boundary sits an order of magnitude below the weakest probed feature, and a trigger hidden beneath it would have little detector signal to corrupt. Third, the sensitivity study varies one hyperparameter at a time around the adopted configuration, so the insensitivity reported is local to that neighborhood rather than global. Fourth, the FLTrust comparison is a faithful reimplementation on our pipeline rather than the authors' own code, which is the right way to hold the setup fixed but does mean the comparison reflects our reading of the method. Three seeds are sufficient to separate the attack effect from noise and to report a standard deviation, but not to resolve sub-noise differences between neighboring defended variants, and we claim none.

\section{Conclusion}
We showed that accuracy-weighted federated aggregation, a design adopted in recent UAV-FL detection work to reward high-quality clients, is exploitable: two compromised UAV clients of ten can implant a stealthy GPS spoofing backdoor by poisoning a fraction of their data, scaling their updates, and inflating their reported accuracy, raising backdoor lift by up to $0.3036$ while clean accuracy falls by less than two points. We then proposed a server-side behavioral-trust defense that never reads reported accuracy and does not need to know the trigger feature: it probes every discriminative feature, scores each client by its worst anomalous deficit below the cohort median in robust units, and aggregates trust-scaled updates with a coordinate-wise median. Across three seeds the defense eliminates the attacker's advantage (lift $-0.0253$), flags both compromised clients in $100\%$ of client-rounds at a $0.3\%$ honest false-positive rate, generalizes across the discriminative feature set including a mixed trigger, withstands a defense-aware adaptive attacker whose evasion destroys its own backdoor, retains that protection across a sixteen-fold range of its gate parameter, and costs about $1.1\%$ of round time with no client-side work. Benchmarked against FLTrust on an identical pipeline, it removes a backdoor that FLTrust leaves standing at $+0.0787$ lift, and does so without the clean-accuracy cost that FLTrust's magnitude normalization imposes on honest clients. Extending the evaluation to a second, independent signal domain and to non-IID fleets is the primary direction for future work.
