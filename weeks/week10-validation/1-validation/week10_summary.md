...
4. **No optimizing adaptive attacker.** A separate study (`../3-adaptive-attacker/`) tested the natural adaptive strategy, poisoning less to stay under the dead-zone, and found no profitable operating point: at the poison rate low enough to evade detection, the undefended backdoor is already worth +0.0051, indistinguishable from zero. That does not cover an attacker who explicitly optimizes against the probe, for example by adding a probe-slice penalty to its local loss. That remains the strongest threat to this defense.
